...
{	
	...
	for(uint8_t i=0;i<8;i++) {			  
		LCD_SCLK_Clr();
		__NOP(); __NOP(); __NOP(); __NOP(); __NOP();  /* 延时 ~100ns @480MHz, 让 MOSI 稳定 */
		if(dat&0x80) {
		   LCD_MOSI_Set();
		} else {
		   LCD_MOSI_Clr();
		}
		__NOP(); __NOP(); __NOP(); __NOP(); __NOP();  /* 延时 ~100ns */
		LCD_SCLK_Set();
		__NOP(); __NOP(); __NOP(); __NOP(); __NOP();  /* 延时 ~100ns, 确保数据锁存 */
		dat<<=1;
	}
#else
...
void LCD_ShowFloatNum(uint16_t x, uint16_t y, float num, uint8_t len, uint8_t decimal, uint16_t fc, uint16_t bc, uint8_t sizey)
{
    ...
    sizex = sizey / 2;
    num_int = (int32_t)(num * (float)mypow(10, decimal));
    ...
    // mode=0 写字符时已画背景像素，无需 LCD_Fill（避免阻塞电机1kHz时序）
    for (t = 0; t < len; t++)
    {
        if (t == (len - decimal))
        {
